...
def calcul_zones(params_dict):
    ...
    measurement_strategy = params_dict.get('measurement_strategy', 'excluded')
    grinding_strategy = params_dict.get('grinding_strategy', 'out_of_scope')
    ...
    if feeding_system == 'third_rail':
        ...
        V_isolator_third_rail = params_dict['V_isolator_3rd_rail']
        V_power_feed_third_rail = params_dict['V_power_feed_3rd_rail']
        ...
        third_rail_isolator_zones = generer_zones_third_rail('Maintenance isolator', V_isolator_third_rail, suffix='Conductor_Isol')
        third_rail_power_feed_zones = generer_zones_third_rail('Maintenance power feed', V_power_feed_third_rail, suffix='Conductor_Feeder')
        third_rail_zones += third_rail_expansion_joint_zones
        third_rail_zones += third_rail_isolator_zones
        third_rail_zones += third_rail_power_feed_zones
    ...
    # Zones grinding
    if grinding_strategy != 'out_of_scope' and V_grinding_Vehicle:
    ...
    # définition des zones d'inspections au dépot
...
def construire_track_df(zones, type_track_installation, cleaning_strategy, grinding_strategy, drainage_cleaning_strategy,
                        measurement_strategy,lubrication_strategy,
    ...
    equipments = ['Track_foot', 'Track_railroad']

    if grinding_strategy != 'out_of_scope':
        equipments.append('Track_grinding')
...
def read_and_strip_all_text_cells(file_path: str, sheet_name: str, engine="openpyxl") -> pd.DataFrame:
    try:
        ...
        print(f"Reading '{sheet_name}' failed ({e}). Using an empty DataFrame.")
        return pd.DataFrame()
...
def core_process(input_file=None, output_file=None, *, silent=False):
    ...
    try: 
        ...
        track_df = construire_track_df(
            zones, type_track_installation, cleaning_strategy, grinding_strategy, drainage_cleaning_strategy, measurement_strategy, lubrication_strategy,
            diamond_crossing_zones, crossover_zones, switch_zones, switch_depot_zones, switch_depot_count,
            IRJ_zones, IRJ_zone_count, REJ_zones, REJ_zone_count,
            buffer_stop_zones, buffer_stop_zone_count,
            depot_zones, buffer_stop_depot_count, wheel_stop_count, insulated_rail_joint_depot_count,
        ...
        if feeding_system == 'third_rail':
            ...
            third_rail_equipments = ['3rd_rail_foot', 'third_rail_expansion_joint', 'third_rail_isolator', 'third_rail_power_feed']
            if third_rail_measurement_strategy != 'excluded':
            ...
            for zone_info in third_rail_zones:
                ...
                elif inspection_method == 'Maintenance isolator':
                    values[third_rail_equipments.index('third_rail_isolator')] = 1

                elif inspection_method == 'Maintenance power feed':
                    values[third_rail_equipments.index('third_rail_power_feed')] = 1
        ...
        ultrasonic_inspection_strategy = params_dict.get('ultrasonic_inspection_strategy', 'excluded')
        ...
        if measurement_strategy != 'excluded':
        ...
        # Activity 15
        rail_expansion_joint_count = int(params_dict.get('rail_expansion_joint', 0))
        if rail_expansion_joint_count > 0:
            activities.append({
                'subsystem': 'Track',
        ...
        # Activity 16
        insulated_rail_joint_count = int(params_dict.get('insulated_rail_joint', 0))
        if insulated_rail_joint_count > 0:
            activities.append({
                'subsystem': 'Track',
        ...
        if ultrasonic_inspection_strategy != 'excluded':
        ...
        activities.append({
            'subsystem': 'Track',
            'equipment': 'Turnout',
            'description': 'Visual inspection of line turnouts and third rail air gaps kit',
            'time_of_day': 'night',
            'periodicity_weeks': 4,
            'duration_hours': 0.5 if params_dict.get('point_machine', 'excluded') == 'excluded' else 0.67,
            'outage_time_before': 0,
            'outage_time_after': 0,
            'technicians_needed': 2
        })
        ...
        activities.append({
            ...
            'time_of_day': 'night',
            'periodicity_weeks': 13,
            'duration_hours': 0.67 if params_dict.get('point_machine', 'excluded') == 'excluded' else 0.75,
            'outage_time_before': 0,
            'outage_time_after': 0,
            'technicians_needed': 2
        })
        ...
        activities.append({
            'subsystem': 'Track',
            'equipment': 'Turnout_depot',
            'description': 'Visual inspection of depot turnouts',
            'time_of_day': 'day',
            'periodicity_weeks': 13,
            'duration_hours': 0.5 if params_dict.get('point_machine', 'excluded') == 'excluded' else 0.67,
            'outage_time_before': 0,
        ...
        if feeding_system == 'third_rail':
            ...
            activities.append({
                'subsystem': '3rd_Rail',
                'equipment': 'third_rail_isolator',
                'description': 'Maintenance of isolators of the line feeding system',
                'time_of_day': 'night',
                'periodicity_weeks': 52,
                'duration_hours': D_inspection_disponible,
                'outage_time_before': T_hors_tension,
            ...
            activities.append({
                'subsystem': '3rd_Rail',
                'equipment': 'third_rail_power_feed',
                'description': 'Maintenance of Power feeds of the line feeding system',
                'time_of_day': 'night',
                'periodicity_weeks': 52,
                'duration_hours': D_inspection_disponible,
                'outage_time_before': T_hors_tension,
