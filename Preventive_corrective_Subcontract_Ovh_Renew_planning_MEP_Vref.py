...
def core_process(input_file=None, output_file=None, *, silent=False):
    ...
    try:    
        ...
        _NEEDED_SHEETS = [
            ...
            'corrective_activities', 'ovh_renew_activities',
        ]
        ...
        def _concat_no_duplicates(df_base, df_src):
            ...
            if df_src.empty:
                return df_base.copy()
            # Normaliser les noms de colonnes en strings (évite les float column names)
            ...
            # Convertir chaque valeur en str explicitement pour éviter l'erreur sur float/NaN
            def _row_key(row):
        ...
        def _run_concat_ui():
            ...
            # La colonne 1 (Entry) s'étire, les colonnes 0 et 2 restent fixes
        ...
        if do_concat:
            ...
            if not silent:
                show_message(
                    ...
                    "'Project_name_Combined_Preventive_corrective_Ovh_Renew_inputs_Vxx'.")
        ...
        # Lire les données de la feuille 'ovh_renew_activities'
        ovh_renew_activities = read_sheet_strip_cells_or_empty(file_path, 'ovh_renew_activities')    
        if ovh_renew_activities.empty:
            print("No overhaul or renewal activities to schedule.")
            ovh_renew_activities = None
        ...
        subsystems = pd.Index(pd.unique(pd.concat([
            ...
            uniques_or_empty(ovh_renew_activities, 'subsystem'),
        ], ignore_index=True)))
        ...
        # Planification des activités Overhaul/Renewal

        all_ovh_renew_plannings = []
        ...
        for subsystem in subsystems:
            ...
            # Ajouter la planification Overhaul et Renewal pour le sous-système courant
            filtered_ovh_renew_activities = (
                ovh_renew_activities[
                    (ovh_renew_activities['subsystem'] == subsystem) &
                    (ovh_renew_activities['span_life'] <= contract_duration_years)
                ]
                if ovh_renew_activities is not None and {'subsystem','span_life'}.issubset(ovh_renew_activities.columns)
                else pd.DataFrame()
            )

            # Retirer les activités Overhaul/Renewal avec quantité nulle (ou manquante)
            if not filtered_ovh_renew_activities.empty and 'quantity_system_level' in filtered_ovh_renew_activities.columns:
                filtered_ovh_renew_activities = filtered_ovh_renew_activities[
                    filtered_ovh_renew_activities['quantity_system_level'].fillna(0) > 0
                ]
            ...
            # Ajouter une colonne pour distinguer les années génériques
            ...
            if not filtered_corrective_activities.empty:
                for idx, row in filtered_corrective_activities.iterrows():
                    ...
                    for year, (replacements, replacements_non_arrondi_per_year) in enumerate(zip(replacements_needed, replacements_non_arrondi), start=start_year):
                        ...
                        # Vérifier si total_replacement_hours est valide
            ...
            # Planning des activites overhaul and renewals   
            for idx, row in filtered_ovh_renew_activities.iterrows():
                equipment = row['equipment']
                element = row['element']
                activity_type = row['type']
                span_life = row['span_life']
                quantity_system_level = row['quantity_system_level']
                replacement_time = row['replacement_time']
                material_unit_price = row['material_unit_price']
                management_cost = row['management_cost']
                t_and_c_cost = row['T&C_cost']
                ...
                # Commencer à planifier à partir de planning_year + span_life
                first_year = start_year + span_life
                ...
                for i in range(num_replacements):
                    ...
                    # Calculer les coûts et la durée totale
                    total_replacement_time = replacement_time * quantity_system_level
                    material_cost = material_unit_price * quantity_system_level
                    global_cost = material_cost + management_cost + t_and_c_cost
                    
                    # Ajouter une ligne au planning
                    ovh_renew_planning.append({
                        ...
                        'Type': activity_type,
                        'Span Life': span_life,
                        'Total Replacement Time': total_replacement_time,
                        'Global Cost': global_cost,
                        'Material Cost': material_cost,
                        'Management Cost': management_cost,
                        'T&C Cost': t_and_c_cost,
                        'Staff Needed': staff_needed,
                        'Currency': currency,
                        'Source': source
                    })
                    
            # Ajouter les résultats au planning global Overhaul/Renewal
            if ovh_renew_planning:
                all_ovh_renew_plannings.append(pd.DataFrame(ovh_renew_planning))         
        ...
        # Combiner les plannings Overhaul et Renewal si des activités existent
        if all_ovh_renew_plannings:
            combined_ovh_renew_df = pd.concat(all_ovh_renew_plannings, ignore_index=True)
        else:
            combined_ovh_renew_df = pd.DataFrame(columns=[
                'Year of Planning', 'Subsystem', 'Equipment', 'Element', 'Type',
                'Span Life', 'Total Replacement Time', 'Global Cost', 'Material Cost',
                'Management Cost', 'T&C Cost', 'Staff Needed', 'Currency', 'Source'
            ])
        ...
        if not combined_ovh_renew_df.empty:
            if overhaul_display_from_year is not None:
                combined_ovh_renew_df = combined_ovh_renew_df[
                    combined_ovh_renew_df['Year of Planning'] >= overhaul_display_from_year
                ]
            if overhaul_display_to_year is not None:
                combined_ovh_renew_df = combined_ovh_renew_df[
                    combined_ovh_renew_df['Year of Planning'] <= overhaul_display_to_year
                ]

            # Synthèse des données Overhaul et Renewal
        
            # Par sous-système et type (Overhaul ou Renewal)
            for (subsystem, activity_type), filtered_df in combined_ovh_renew_df.groupby(['Subsystem', 'Type'], dropna=False):
                filtered_df = combined_ovh_renew_df[
                    (combined_ovh_renew_df['Subsystem'] == subsystem) & 
                    (combined_ovh_renew_df['Type'] == activity_type)
                ]

                # Calculer les totaux et le maximum pour la synthèse
                total_replacement_time = filtered_df['Total Replacement Time'].sum()
                max_staff_needed = filtered_df['Staff Needed'].max()

                # Ajouter les données au tableau de synthèse
                synthesis_data.append({
                    'Subsystem': subsystem,
                    'Type': activity_type,
                    'Shift': None,
                    'Total Replacement Time': round(total_replacement_time, 2),
                    'Staff Needed (Overhaul/Renewal)': max_staff_needed
                })

                # Par devise (Currency)
                for currency in filtered_df['Currency'].unique():
                    filtered_currency_df = filtered_df[filtered_df['Currency'] == currency]

                    # Calculer les sommes des coûts
                    total_global_cost = filtered_currency_df['Global Cost'].sum()
                    total_management_cost = filtered_currency_df['Management Cost'].sum()
                    total_tc_cost = filtered_currency_df['T&C Cost'].sum()
                    total_material_cost = filtered_currency_df['Material Cost'].sum()
                    ...
                    synthesis_data.append({
                        ...
                        'Total Management Cost': round(total_management_cost, 2),
                        'Total T&C Cost': round(total_tc_cost, 2),
                        'Total Material Cost': round(total_material_cost, 2)
                    })
        ...
        try:
            with pd.ExcelWriter(output_file) as writer:
                ...
                combined_ovh_renew_df.to_excel(writer, sheet_name='Overhaul and Renewal Planning', index=False)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Maintenance planning + Ovh/Renew")
